...
class Regression:
    ...
    def perform(self):
        ...
        if self.N <= self.effective_training_count:
            ...
            self.Y = np.array(self.Y, dtype=np.float_)
...
class EvidentialSupport:
    ...
    def __init__(self, variables, features, class_num):
        '''
        @param variables:
        @param features:
        '''
    ...
    def separate_feature_value(self, c):
        ...
        for feature in self.features:
            ...
            if 'class_pro' in feature['feature_name']:
                _1_pro_window_feature_easys = []
                for var_id, value in feature['weight'].items():
                    if type(var_id) == int:
                        if var_id in self.observed_variables_set:
                            _1_pro_window_feature_easys.append([value[1], (1 if self.variables[var_id]['label'] == feature['Association_category'] else 0)])
                self.features_easys[feature['feature_id']] = copy(_1_pro_window_feature_easys)
            ...
            if feature['parameterize'] == 1 and feature['feature_type'] == 'binary_feature':
                ...
                for var_id, value in feature['weight'].items():
                    if type(var_id) == tuple:
                        ...
                        if self.variables[var_id[0]]['is_evidence'] and self.variables[var_id[1]]['is_evidence']:  # 判断两个是不是证据集变量中
    ...
    def evidential_support_by_regression(self, variable_set, update_feature_set, c=0):
        ...
        if len(cur_update_set) != 0:
            ...
            logunes = np.log(1 - np.dot(evidential_support, 0.999999))
    ...
    def ev_by_regression(self, update_feature_set, c=-1):
        ...
        cur_update_set -= del_set
        self.influence_modeling(cur_update_set)
        binary_approximate_weight = np.zeros(shape=(len(self.variables), self.classNum), dtype=np.float_)  # 赋给binary_approximate_weight全为0的矩阵[100,5]
            ...
            if feature['feature_type'] == 'binary_feature':
                ...
                if regression != None and zero_confidence == 1 and np.isnan(Ns) == False and np.isnan(
                    ...
                    for weight in feature['weight']:
                        ...
                        tvalue = float(delta) / ((residuals * np.sqrt(1 + 1.0 / Ns + np.power(featurevalue - meanX, 2) / variance)) + 0.000000001)
